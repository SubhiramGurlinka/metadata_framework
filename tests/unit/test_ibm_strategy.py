...
def test_process_flow(mock_get, ibm_strategy, mock_parser):
    ...
    # Patch URL resolution at strategy level
    with patch.object(ibm_strategy, 'get_url', return_value="http://test.com"):
        results = ibm_strategy.process("mq", "9.1", "9.1.0.5")
    ...
    assert passed_context["product_fix_version"] == "9.1.0.5"
...
def test_process_network_failure(mock_get, ibm_strategy):
    ...
    with patch.object(ibm_strategy, 'get_url', return_value="http://test.com"):
        with pytest.raises(requests.exceptions.Timeout):
            ibm_strategy.process("mq", "9.1", "9.1.0.5")